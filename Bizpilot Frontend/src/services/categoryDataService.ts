...
export async function getCategoryConfig(): Promise<CategoryConfigResponse> {
  const response = await apiFetch(`${API_BASE_URL}/business/category-data/config`, {
    method: "GET",
  });
...
export async function getCategoryRows(): Promise<CategoryRowResponse[]> {
  const response = await apiFetch(`${API_BASE_URL}/business/category-data/rows`, {
    method: "GET",
  });
  const data = await response.json();
...
export async function uploadCategoryItemImage(file: File): Promise<string> {
  ...
  const response = await apiFetch(`${API_BASE_URL}/business/category-data/image`, {
    method: "POST",
    body: formData,
  });
  const data = await response.json();
